...
<div class="max-w-[1400px] mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Gestión de Fotógrafos</h2>
            <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mt-1">
                <span>Fototeca</span>
                <i data-lucide="chevron-right" class="w-3 h-3"></i>
                <span class="text-slate-700 dark:text-slate-200">Fotógrafos</span>
            </div>
        </div>
    ...
    <div class="bulk-wrapper bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border overflow-hidden">
        ...
        <div class="p-5 border-b border-slate-100 dark:border-dark-border flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-dark-surface">
            <form method="GET" action="{{ route('admin.fototeca.photographers') }}" class="relative w-full sm:w-80">
                <i data-lucide="search" class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none"></i>
                <input id="search-input" name="search" type="text" value="{{ $q ?? '' }}" placeholder="Buscar fotógrafos..."
                ...
                @if(!empty($q))
                <a href="{{ route('admin.fototeca.photographers') }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200" title="Limpiar búsqueda">
                    <i data-lucide="x" class="w-4 h-4"></i>
                </a>
                @endif
            </form>
            <span class="text-sm text-slate-500 dark:text-slate-400">{{ $photographers->total() }} registros</span>
        </div>
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group photo-row">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form action="{{ route('admin.fototeca.photographers.destroy', $p) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                        onclick="confirmDelete(this.closest('form'), '¿Eliminar este fotógrafo?'); return false;"
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center gap-3 text-slate-400 dark:text-slate-500">
                                ...
                                <p class="text-sm font-medium">No hay fotógrafos registrados</p>
                                <a href="{{ route('admin.fototeca.photographers.create') }}" class="text-xs text-brand-600 dark:text-brand-400 hover:underline">Agregar el primero →</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <x-admin-pagination :paginator="$photographers" />
        @endif
    </div>
</div>
@endsection
