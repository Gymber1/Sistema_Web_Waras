...
@section('section', 'Fototeca > 1er Nivel')
...
<div class="max-w-[900px] mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mt-1">
                ...
                <span class="text-slate-700 dark:text-slate-200">1er Nivel</span>
            </div>
        </div>
    ...
    <div class="bulk-wrapper bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border overflow-hidden">
        ...
        <div class="p-5 border-b border-slate-100 dark:border-dark-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <span class="text-sm text-slate-500 dark:text-slate-400">{{ $sublevels->total() }} 1er niveles</span>
            <form method="GET" action="{{ route('admin.fototeca.sublevels') }}" class="relative w-full sm:w-72">
                <i data-lucide="search" class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none"></i>
                <input id="search-input" name="search" type="text" value="{{ $q ?? '' }}" placeholder="Buscar 1er nivel..."
                ...
                @if(!empty($q))
                <a href="{{ route('admin.fototeca.sublevels') }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200" title="Limpiar búsqueda">
        ...
        <div class="overflow-x-auto">
            <table id="table-sublevels" class="w-full text-left text-sm">
                <thead class="bg-slate-50/80 dark:bg-slate-800/40 border-b border-slate-200 dark:border-dark-border">
                    <tr>
                        ...
                        <x-sort-th column="parent" label="Subcategoría" />
        ...
        <x-admin-pagination :paginator="$sublevels" />
    </div>
</div>
@endsection
