...
@section('section', 'Fototeca > Colecciones > Editar')
...
<div class="max-w-[600px] mx-auto">
    ...
    <div class="mb-6 flex items-start gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Modifica los campos y guarda los cambios.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.fototeca.collections.update', $special) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        <div class="bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border p-6 md:p-8 space-y-6">
            ...
            <div class="flex items-center gap-4 p-4 bg-slate-50/80 dark:bg-slate-800/30 rounded-lg border border-slate-100 dark:border-dark-border">
                <img src="{{ Storage::url($special->cover_image_path) }}"
                    class="w-16 h-16 object-cover rounded-lg border border-slate-200 dark:border-slate-700 cursor-zoom-in"
                    onclick="openLightbox('{{ Storage::url($special->cover_image_path) }}', '{{ addslashes($special->title) }}')"
                    onerror="this.style.display='none'">
                <div>
                    <p class="font-semibold text-slate-800 dark:text-white text-sm">Portada actual</p>
                    <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Sube una nueva imagen para reemplazarla.</p>
                </div>
            </div>
            @endif
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $special->title) }}" required autofocus
            ...
            <div>
                ...
                <x-searchable-select
                    ...
                    :selected="old('featured_photographer', $special->description ?? '')"
            ...
            <div>
                ...
                <x-searchable-select
                    ...
                    :selected="old('featured_donor', $special->featured_donor ?? '')"
            ...
            <div>
                ...
                <p class="text-xs text-slate-400 dark:text-slate-500 mt-1.5">Opcional. Deja en blanco para conservar la actual. Máx. 5MB.</p>
            </div>
        </div>
