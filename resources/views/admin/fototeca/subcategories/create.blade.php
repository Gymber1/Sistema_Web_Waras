...
@section('section', 'Fototeca > Subcategorías > Nueva')
...
<div class="max-w-[600px] mx-auto">
    ...
    <div class="mb-6 flex items-start gap-4">
        ...
        <div>
            <div class="flex items-center gap-3 flex-wrap">
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Agregar Subcategoría</h2>
                <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-medium bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400 border border-brand-100 dark:border-brand-500/20">NIVEL 2</span>
            </div>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Elige la categoría principal a la que pertenece esta subcategoría.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.fototeca.subcategories.store') }}" method="POST">
        @csrf
        ...
        <div class="mt-5 flex gap-3 justify-end">
            ...
            <button type="submit"
                class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium shadow-lg shadow-emerald-500/20 transition-all">Guardar subcategoría</button>
        </div>
    </form>
</div>
@endsection
