...
@section('section', 'Fototeca > Subcategorías')
...
<div class="max-w-[900px] mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Subcategorías</h2>
            <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mt-1">
                <span>Fototeca</span>
                <i data-lucide="chevron-right" class="w-3 h-3"></i>
                <span class="text-slate-700 dark:text-slate-200">Subcategorías</span>
            </div>
        </div>
        <a href="{{ route('admin.fototeca.subcategories.create') }}"
    ...
    <div class="bulk-wrapper bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border overflow-hidden">
        ...
        <div class="p-5 border-b border-slate-100 dark:border-dark-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <span class="text-sm text-slate-500 dark:text-slate-400">{{ $subcategories->total() }} subcategorías</span>
            <form method="GET" action="{{ route('admin.fototeca.subcategories') }}" class="relative w-full sm:w-72">
                <i data-lucide="search" class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none"></i>
                <input id="search-input" name="search" type="text" value="{{ $q ?? '' }}" placeholder="Buscar subcategoría..."
                ...
                @if(!empty($q))
                <a href="{{ route('admin.fototeca.subcategories') }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200" title="Limpiar búsqueda">
        ...
        <div class="overflow-x-auto">
            <table id="table-foto-subcategories" class="w-full text-left text-sm">
                <thead class="bg-slate-50/80 dark:bg-slate-800/40 border-b border-slate-200 dark:border-dark-border">
                    <tr>
                        ...
                        <th class="px-6 py-4 font-semibold text-[11px] text-slate-500 dark:text-slate-400 uppercase tracking-wider">Categoría padre</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-dark-border">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group sub-row">
                        <td class="px-4 py-4"><input type="checkbox" class="row-check" value="{{ $cat->id }}"></td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <i data-lucide="chevron-right" class="w-3 h-3 text-brand-400 shrink-0"></i>
                                <span class="font-semibold text-slate-800 dark:text-white sub-name">{{ $cat->name }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('admin.fototeca.subcategories.edit', $cat) }}"
                                ...
                                <form action="{{ route('admin.fototeca.subcategories.destroy', $cat) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                        onclick="confirmDelete(this.closest('form'), '¿Eliminar «{{ addslashes($cat->name) }}» y todos sus subniveles?'); return false;"
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center gap-3 text-slate-400 dark:text-slate-500">
                                ...
                                <p class="text-sm font-medium">No hay subcategorías registradas</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
