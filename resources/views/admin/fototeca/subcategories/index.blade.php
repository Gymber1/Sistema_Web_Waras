...
@section('section', 'Fototeca > SubCategoría')
...
<div class="p-6 md:p-10 max-w-[1200px] mx-auto">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            ...
            <p class="text-slate-500 mt-1">Segundo nivel de clasificación. Cada subcategoría pertenece a una categoría principal.</p>
        </div>
        <a href="{{ route('admin.fototeca.subcategories.create') }}"
            class="flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-cyan-200 transition-all whitespace-nowrap">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
            Agregar SubCategoría
        </a>
    </div>

    <div class="bg-white rounded-2xl shadow-sm overflow-hidden border-t-4 border-cyan-500">
        <div class="p-5 border-b border-slate-100 flex items-center justify-between">
            <span class="text-sm font-bold bg-cyan-50 text-cyan-700 px-4 py-2 rounded-lg">{{ $subcategories->count() }} subcategorías</span>
            <input type="text" id="search-sub" placeholder="Buscar subcategoría..."
                oninput="filterTable(this.value)"
                class="px-4 py-2 text-sm border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-cyan-400 w-64">
        </div>
                    ...
                    <tr class="hover:bg-slate-50 group transition-colors" data-name="{{ strtolower($cat->name) }}">
                        <td class="py-3 px-6">
                            <div class="flex items-center gap-2">
                                <svg class="w-3 h-3 text-cyan-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                                <span class="text-sm font-semibold text-slate-700">{{ $cat->name }}</span>
                            </div>
                        </td>
                        <td class="py-3 px-6 text-sm text-slate-500">
                            <span class="px-2.5 py-1 bg-blue-50 text-blue-700 rounded-lg text-xs font-bold">{{ $cat->parent?->name ?? '—' }}</span>
                        </td>
