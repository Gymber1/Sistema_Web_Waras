...
@section('section', 'Fototeca > Donadores > Editar')

@section('content')
<div class="max-w-[720px] mx-auto">

    <div class="mb-6 flex items-start gap-4">
        <a href="{{ route('admin.fototeca.donors') }}"
        ...
        <div>
            <div class="flex items-center gap-3 flex-wrap">
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Editar donador</h2>
                <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-medium bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400 border border-brand-100 dark:border-brand-500/20">DONADOR</span>
            </div>
    ...
    <form action="{{ route('admin.fototeca.donors.update', $donor) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        ...
        <div class="bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border p-6 md:p-8 space-y-6">
            ...
            <div class="flex items-center gap-4 p-4 bg-slate-50/80 dark:bg-slate-800/30 rounded-lg border border-slate-100 dark:border-dark-border">
                <div class="w-14 h-14 rounded-full overflow-hidden border-2 border-slate-200 dark:border-slate-600 flex-shrink-0 cursor-zoom-in"
                    @if($donor->photo_path) onclick="openLightbox('{{ Storage::url($donor->photo_path) }}', '{{ addslashes($donor->full_name) }}')" @endif>
                    @if($donor->photo_path)
                    <img src="{{ Storage::url($donor->photo_path) }}" class="w-full h-full object-cover" onerror="this.parentElement.className='w-14 h-14 rounded-full bg-brand-50 dark:bg-brand-500/10 flex items-center justify-center text-brand-500 font-bold text-xl'">
                    @else
                    <div class="w-full h-full bg-brand-50 dark:bg-brand-500/10 flex items-center justify-center text-brand-500 font-bold text-xl">{{ mb_substr($donor->full_name, 0, 1) }}</div>
                    @endif
                </div>
                <div>
                    <p class="font-semibold text-slate-800 dark:text-white">{{ $donor->full_name }}</p>
                    <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">ID #{{ $donor->id }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="full_name" value="{{ old('full_name', $donor->full_name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="birth_place" value="{{ old('birth_place', $donor->birth_place) }}" placeholder="Ej. Huaraz, Áncash"
                        class="w-full px-4 py-2.5 bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 outline-none transition-all">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Fecha de nacimiento</label>
                    <input type="date" name="birth_date" value="{{ old('birth_date', $donor->birth_date?->format('Y-m-d')) }}"
                        class="w-full px-4 py-2.5 bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 outline-none transition-all">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Lugar de fallecimiento</label>
                    <input type="text" name="death_place" value="{{ old('death_place', $donor->death_place) }}" placeholder="Dejar vacío si aún vive"
                        class="w-full px-4 py-2.5 bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 outline-none transition-all">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Fecha de fallecimiento</label>
                    <input type="date" name="death_date" value="{{ old('death_date', $donor->death_date?->format('Y-m-d')) }}"
                        class="w-full px-4 py-2.5 bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 outline-none transition-all">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Foto del donador</label>
                    @if($donor->photo_path)
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="biography" rows="4"
                        class="w-full px-4 py-2.5 bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 outline-none transition-all resize-y">{{ old('biography', $donor->biography) }}</textarea>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Crítica / Estudios</label>
                    <textarea name="studies_critique" rows="3" placeholder="Análisis, publicaciones o crítica académica sobre el donador..."
                        class="w-full px-4 py-2.5 bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 outline-none transition-all resize-y">{{ old('studies_critique', $donor->studies_critique) }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div id="chips-collections" class="flex flex-wrap gap-2 p-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800/50 min-h-[48px]">
                        @foreach($donor->collections as $col)
