...
@section('section', 'Fototeca > 3er Nivel')
...
<div class="max-w-[900px] mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white">3er Nivel</h2>
            <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mt-1">
                <span>Fototeca</span>
                <i data-lucide="chevron-right" class="w-3 h-3"></i>
                <span class="text-slate-700 dark:text-slate-200">3er Nivel</span>
            </div>
        </div>
        <a href="{{ route('admin.fototeca.thirdlevels.create') }}"
    ...
    <div class="bulk-wrapper bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border overflow-hidden">
        ...
        <div class="p-5 border-b border-slate-100 dark:border-dark-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <span class="text-sm text-slate-500 dark:text-slate-400">{{ $thirdlevels->total() }} 3er niveles</span>
            <form method="GET" action="{{ route('admin.fototeca.thirdlevels') }}" class="relative w-full sm:w-72">
                <i data-lucide="search" class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none"></i>
                <input id="search-input" name="search" type="text" value="{{ $q ?? '' }}" placeholder="Buscar 3er nivel..."
                ...
                @if(!empty($q))
                <a href="{{ route('admin.fototeca.thirdlevels') }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200" title="Limpiar búsqueda">
        ...
        <div class="overflow-x-auto">
            <table id="table-thirdlevels" class="w-full text-left text-sm">
                <thead class="bg-slate-50/80 dark:bg-slate-800/40 border-b border-slate-200 dark:border-dark-border">
                    <tr>
                        ...
                        <th class="px-6 py-4 font-semibold text-[11px] text-slate-500 dark:text-slate-400 uppercase tracking-wider">2do Nivel padre</th>
                        <th class="px-6 py-4 font-semibold text-[11px] text-slate-500 dark:text-slate-400 uppercase tracking-wider">1er Nivel</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-dark-border">
                    @forelse($thirdlevels as $cat)
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group thr-row">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-semibold text-slate-800 dark:text-white thr-name">{{ $cat->name }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('admin.fototeca.thirdlevels.edit', $cat) }}"
                                ...
                                <form action="{{ route('admin.fototeca.thirdlevels.destroy', $cat) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center gap-3 text-slate-400 dark:text-slate-500">
                                ...
                                <p class="text-sm font-medium">No hay 3er niveles registrados</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <x-admin-pagination :paginator="$thirdlevels" />
    </div>
</div>
@endsection
