...
@section('section', 'Fototeca > 2do Nivel > Nuevo')
...
<div class="max-w-[600px] mx-auto">
    ...
    <div class="mb-6 flex items-start gap-4">
        ...
        <div>
            <div class="flex items-center gap-3 flex-wrap">
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Agregar 2do Nivel</h2>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Elige el 1er nivel al que pertenece este 2do nivel.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.fototeca.secondlevels.store') }}" method="POST">
        @csrf
        <div class="bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border p-6 md:p-8 space-y-6">
            ...
            <x-searchable-select
                name="parent_id"
                label="1er Nivel padre"
                :required="true"
                placeholder="— Seleccionar 1er nivel —"
                :selected="old('parent_id', '')"
                :options="$parentCategories->map(fn($c) => ['value'=>$c->id, 'text'=>($c->parent?->parent?->name ? $c->parent->parent->name.' › ' : '').($c->parent?->name ? $c->parent->name.' › ' : '').$c->name])" />
        </div>
        <div class="mt-5 flex gap-3 justify-end">
            ...
            <button type="submit"
                class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium shadow-lg shadow-emerald-500/20 transition-all">Guardar 2do nivel</button>
        </div>
    </form>
</div>
@endsection
