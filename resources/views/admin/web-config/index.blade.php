        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="p-6 flex flex-col lg:flex-row gap-6">
                ...
                <div class="lg:w-72 shrink-0">
                    ...
                    <div class="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-100" style="aspect-ratio:16/7;" id="preview-box-{{ $key }}">
                        @if($site['value'])
                            <img src="{{ Storage::url($site['value']) }}"
                                 alt="Fondo {{ $site['label'] }}"
                        ...
                        @else
                            ...
                            {{-- Imagen oculta para preview antes de guardar --}}
                ...
                <div class="flex-1 flex flex-col gap-4">
                    ...
                    {{-- FORM UPLOAD (fuera del form borrar) --}}
                    <form method="POST"
                          action="{{ route('admin.web-config.update', $key) }}"
                        ...
                        <div class="flex items-center gap-3 mt-4 flex-wrap">
                            ...
                            @if($site['value'])
                            {{-- BOTÓN BORRAR: form separado, FUERA del form upload --}}
                            <button type="button"
                                    onclick="deleteBg('{{ $key }}', '{{ $site['label'] }}')"
...
{{-- Forms de borrado separados (fuera de cualquier otro form) --}}
@foreach($settings as $key => $site)
    @if($site['value'])
    <form method="POST"
          action="{{ route('admin.web-config.destroy', $key) }}"
          id="form-delete-{{ $key }}"
          style="display:none;">
        @csrf
        @method('DELETE')
...
function handleFileSelect(input) {
    ...
    // Mostrar preview en la caja "Vista previa actual"
    ...
    reader.onload = function(e) {
        ...
        const previewEmpty = document.getElementById('preview-empty-' + key);
        const previewOverlay = document.getElementById('preview-overlay-' + key);
