...
@section('section', 'Biblioteca > Revistas > Editar')
...
<div class="max-w-[960px] mx-auto">
    ...
    <div class="mb-6 flex items-start gap-4">
        ...
        <div>
            <div class="flex items-center gap-3 flex-wrap">
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Editar revista</h2>
    ...
    <form action="{{ route('admin.biblioteca.magazines.update', $book) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        ...
        <div class="bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border p-6 md:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <div id="chips-authors" class="flex flex-wrap gap-2 p-3 bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-lg min-h-[48px]">
                ...
                <div class="md:col-span-2">
                    ...
                    <div id="chips-descriptors" data-sortable="1" class="flex flex-wrap gap-2 p-3 bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-lg min-h-[48px]">
                        @foreach($book->descriptors as $descriptor)
                        <span class="chip inline-flex items-center gap-1.5 px-3 py-1.5 bg-brand-50 dark:bg-brand-500/10 text-brand-700 dark:text-brand-300 text-xs font-semibold rounded-full border border-brand-100 dark:border-brand-500/20" draggable="true">
                            <i data-lucide="grip-vertical" class="w-3 h-3 opacity-40 cursor-grab drag-handle"></i>
                            {{ $descriptor->name }}
                            <button type="button" onclick="removeChip(this)" class="hover:text-red-500 font-bold text-sm leading-none">×</button>
                ...
                <div id="field-pdf" style="display:none">
                    ...
                    <div class="flex items-center gap-3 mb-3 p-3 bg-brand-50 dark:bg-brand-500/10 border border-brand-100 dark:border-brand-500/20 rounded-lg">
                        <i data-lucide="file-text" class="w-5 h-5 text-brand-500 shrink-0"></i>
                        <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex items-center gap-3 mb-3 p-3 bg-brand-50 dark:bg-brand-500/10 border border-brand-100 dark:border-brand-500/20 rounded-lg">
                        <img src="{{ Storage::url($book->cover_image_path) }}"
                            class="w-10 h-14 object-cover rounded-md border border-brand-200 dark:border-brand-500/30"
...
function addChip(btn, chipsId, field) {
    ...
    chip.innerHTML = `${handle}${name} <button type="button" onclick="removeChip(this)" class="hover:text-red-500 font-bold text-sm leading-none">×</button><input type="hidden" name="${field}[]" value="${id}">`;
