...
@section('section', 'Biblioteca > SubCategorías Revistas')
...
<div class="max-w-[900px] mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white">SubCategorías de Revistas</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Segundo nivel de clasificación. Cada subcategoría pertenece a una categoría padre.</p>
        </div>
        <a href="{{ route('admin.biblioteca.revista-subcategories.create') }}"
    ...
    <div class="bulk-wrapper bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table id="table-revista-sub" class="w-full text-left text-sm">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group" data-name="{{ strtolower($cat->name) }}">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('admin.biblioteca.revista-subcategories.edit', $cat) }}"
                                ...
                                <form action="{{ route('admin.biblioteca.revista-subcategories.destroy', $cat) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center gap-3 text-slate-400 dark:text-slate-500">
                                ...
                                <a href="{{ route('admin.biblioteca.revista-subcategories.create') }}" class="text-xs text-brand-600 dark:text-brand-400 hover:underline">Agregar la primera →</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
function filterTable(q) {
    document.querySelectorAll('#table-revista-sub tbody tr[data-name]').forEach(row => {
