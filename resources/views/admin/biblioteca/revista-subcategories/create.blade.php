...
@section('section', 'Biblioteca > SubCategorías Revistas > Nueva')

@section('content')
<div class="max-w-[600px] mx-auto">

    <div class="mb-6 flex items-start gap-4">
        <a href="{{ route('admin.biblioteca.revista-subcategories') }}"
        ...
        <div>
            <div class="flex items-center gap-3 flex-wrap">
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Agregar subcategoría de revista</h2>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Elige la categoría padre a la que pertenece esta subcategoría.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.biblioteca.revista-subcategories.store') }}" method="POST">
        @csrf
        ...
        <div class="bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border p-6 md:p-8 space-y-6">
            ...
            <div>
                ...
                <select name="parent_id" required
                    ...
                    <option value="{{ $cat->id }}" @selected(old('parent_id', request('parent_id')) == $cat->id)>{{ $cat->name }}</option>
                    @endforeach
                </select>
            </div>
