    ...
    <div class="flex items-start gap-4 mb-8">
        ...
        <div>
            <div class="flex items-center gap-3 flex-wrap">
                <h1 class="text-2xl font-black text-slate-900">Editar categoría</h1>
                @php $depth = $category->depth ?? ($category->parent_id ? 1 : 0); @endphp
                <span class="px-3 py-1 bg-emerald-100 text-emerald-700 text-xs font-bold rounded-full uppercase tracking-wider">
                    {{ $depth === 0 ? 'CATEGORÍA' : ($depth === 1 ? 'SUBCATEGORÍA' : 'SUB-SUBCATEGORÍA') }}
                </span>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 outline-none resize-y">{{ old('description', $category->description) }}</textarea>
            </div>
