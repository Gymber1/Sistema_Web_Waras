    ...
    <form action="{{ route('admin.biblioteca.books.update', $book) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border p-6 md:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {{-- Categoría --}}
                @php
                    $allCatIds     = $book->categories->pluck('id')->toArray();
                ...
                <div class="md:col-span-2">
                    ...
                    <div id="chips-descriptors" data-sortable="1" class="flex flex-wrap gap-2 p-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800/50 min-h-[48px]">
                        @foreach($book->descriptors as $descriptor)
                        <span class="chip inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-700/50 text-slate-700 dark:text-slate-300 text-xs font-semibold rounded-full" draggable="true">
