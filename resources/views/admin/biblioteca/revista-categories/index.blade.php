...
@section('section', 'Biblioteca > Categorías Revistas')
...
<div class="max-w-[900px] mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Gestión de Categorías de Revistas</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Categorías principales de revistas (nivel 1). Las subcategorías se gestionan en la sección SubCategorías de Revistas.</p>
        </div>
        <a href="{{ route('admin.biblioteca.revista-categories.create') }}"
    ...
    <div class="bulk-wrapper bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border overflow-hidden">
        ...
        <div class="p-5 border-b border-slate-100 dark:border-dark-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <form method="GET" action="{{ route('admin.biblioteca.revista-categories') }}" class="relative w-full sm:w-72">
        ...
        <div class="overflow-x-auto">
            <table id="table-bib-revista-categories" class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-dark-border">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <span class="font-semibold text-slate-800 dark:text-white">{{ $cat->name }}</span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('admin.biblioteca.revista-categories.edit', $cat) }}"
                                ...
                                <form action="{{ route('admin.biblioteca.revista-categories.destroy', $cat) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="3" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center gap-3 text-slate-400 dark:text-slate-500">
                                ...
                                <a href="{{ route('admin.biblioteca.revista-categories.create') }}" class="text-xs text-brand-600 dark:text-brand-400 hover:underline">Agregar la primera →</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <x-admin-pagination :paginator="$allCategories" />
    </div>
</div>
@endsection
