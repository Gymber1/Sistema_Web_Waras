...
@section('section', 'Biblioteca > 1er Nivel Revistas > Nuevo')

@section('content')
<div class="max-w-[600px] mx-auto">

    <div class="mb-6 flex items-start gap-4">
        <a href="{{ route('admin.biblioteca.revista-firstlevels') }}"
        ...
        <div>
            <div class="flex items-center gap-3 flex-wrap">
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Agregar 1er Nivel de Revista</h2>
                <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-medium bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400 border border-brand-100 dark:border-brand-500/20">1ER NIVEL</span>
            </div>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Elige la subcategoría padre a la que pertenece este 1er nivel.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.biblioteca.revista-firstlevels.store') }}" method="POST">
        @csrf
        ...
        <div class="bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-dark-border p-6 md:p-8 space-y-6">
            ...
            <x-searchable-select
                ...
                :options="$parentCategories->map(fn($c) => ['value' => $c->id, 'text' => ($c->parent?->name ? $c->parent->name.' › ' : '').$c->name])" />
